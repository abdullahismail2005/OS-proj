...
void scheduler_tick(GameState *gs) {
    ...
    if (gs->active_global < 0) {
        for (int i = 0; i < MAX_PLAYERS; ++i) {
            Entity &e = gs->players[i];
            if (!e.active || !e.alive || e.stunned) continue;
            if (e.stamina < e.max_stamina) {
                e.stamina += e.speed;
...
void handle_entity_death(GameState *gs, Entity *dead) {
    ...
    if (!dead->is_player) {
        ...
        if (rand_range(0, 99) < 60) {
            ...
            // picks it up.
            for (int i = 0; i < MAX_PLAYERS; ++i) {
                if (gs->players[i].active && gs->players[i].alive) {
                    gs->pending_drop_weapon = wid;
                    gs->pending_drop_for_player = i;
                    gs->pending_drop_deadline = time(nullptr) + 3;
                    shm_log(gs, "[DROP] %s dropped %s — P%d may pick it up",
                            dead->name, weapon_spec(wid).name, i + 1);
                    break;
                }
            }
        }
...
void apply_action(GameState *gs, Entity *actor) {
    ...
    case ACT_STRIKE: {
        ...
        // Chance to stun on strike: 20%.
        if (rand_range(0, 99) < 20) {
            Entity *victim = actor->is_player
                             ? &gs->enemies[tgt] : &gs->players[tgt];
            if (victim->active && victim->alive) deliver_stun(gs, victim);
        }
        break;
    }
    case ACT_EXHAUST: {
    ...
    case ACT_PICKUP: {
        if (gs->pending_drop_weapon != W_NONE &&
            gs->pending_drop_for_player == actor->local_id &&
            actor->is_player) {
            int inst;
    ...
    case ACT_DECLINE: {
        if (gs->pending_drop_weapon != W_NONE) {
            // Give it to a random alive enemy.
            int candidates[MAX_ENEMIES]; int nc = 0;
            for (int i = 0; i < MAX_ENEMIES; ++i)
                if (gs->enemies[i].active && gs->enemies[i].alive)
                    candidates[nc++] = i;
            if (nc > 0) {
                int ei = candidates[rand_range(0, nc - 1)];
                int inst;
                if (inv_add_weapon(&gs->enemies[ei],
                                   gs->pending_drop_weapon, &inst)) {
                    shm_log(gs, "[PICK] E%d grabbed %s",
                            ei + 1,
                            weapon_spec(gs->pending_drop_weapon).name);
                }
            }
